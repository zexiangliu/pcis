...
sizU = size(U_list,2);
subplot(211)
plot(T_list(1:sizU),U_list(1,:),'r-','linewidth',5);
subplot(212)
plot(T_list(1:sizU),U_list(2,:),'r-','linewidth',5);
%%
U1_list1 = [];
U1_list2 = [];
t1_list = [];
...
%    plot(t,u(1),'.r', 'markersize',15);
   if t < t_detection
       ...
       u1 = preXU_cau.slice([1,2,3,4,6],[x;u(2)]);
   end
   if u1.Num >= 1
       [v1,v2] = find_V(u1);
...
%    plot(t,u(2),'.r', 'markersize',15);
    if t < t_detection
        ...
        u2 = preXU_cau.slice([1,2,3,4,5],[x;u(1)]);
    end
    if u2.Num >= 1
       [v1,v2] = find_V(u2);
...
subplot(211)
  ...
  shade(t1_list,U1_list1,U1_list2);
       plot(t1_list,U1_list1,'-b','linewidth',3);
       plot(t1_list,U1_list2,'-b','linewidth',3);
       plot([t_detection t_detection], [-4 4], '--g', 'LineWidth',3)
