...
% |h| <= h_reaction
Hx_r1 = [ -(con.K_ann*con.dt + [ 0 0 0 1-con.f1*con.dt ]); [ zeros(2,3) [1;-1] ] ]; 
...
% Define Polyhedral domain as Hx * x <= h_x
% Constraints
% K_cau*dt*q + (1-b_L*dt)v_L + delta_max_cau*dt - K3_cau*vL_des*dt >= v_L_max
% |h| <= h_max
Hx_r1 = [   -(con.K_cau*con.dt + [ 0 0 0 1-con.f1*con.dt ]) ;
            [ zeros(2,3) [1;-1] ] ];
hx_r1 = [   -(con.vL_max - con.dLmax*con.dt + con.K3_cau*con.vL_des*con.dt);
            con.h_reaction;
            con.h_reaction];
r1 = Polyhedron('A',Hx_r1,'b',hx_r1);
...
Bw_r2 = { Bw(:,1), Bw(:,2), [zeros(3,1); con.dt] };
% Constraints
% K_cau*dt*q + (1-b_L*dt)v_L + delta_max*dt - K3_cau*vL_des*dt <= v_L_max
% K_cau*dt*q + (1-b_L*dt)v_L + delta_min*dt - K3_cau*vL_des*dt >= v_L_min
Hx_r2 = [   con.K_cau*con.dt + [ 0 0 0 1-con.f1*con.dt ] ;
            -(con.K_cau*con.dt + [ 0 0 0 1-con.f1*con.dt ]) ;
            [ zeros(2,3) [1;-1] ] ];
hx_r2 = [   con.vL_max - con.dLmax*con.dt + con.K3_cau*con.vL_des*con.dt ;
          -(con.vL_min - con.dLmin*con.dt + con.K3_cau*con.vL_des*con.dt);
            con.h_reaction;
            con.h_reaction];
r2 = Polyhedron('A',Hx_r2,'b',hx_r2);
...
Hx_r3 = [   con.K_cau*con.dt + [ 0 0 0 1-con.f1*con.dt ];
            [ zeros(2,3) [1;-1] ] ];
hx_r3 = [   con.vL_min - con.dLmin*con.dt + con.K3_cau*con.vL_des*con.dt;
            con.h_reaction;
            con.h_reaction];
r3 = Polyhedron('A',Hx_r3,'b',hx_r3);
