function [preSets] = pre_rho( varargin )
    ...
    if isa(X,'PolyUnion')
        ...
        for ind_X = 1:X.Num
            temp_preSets{ind_X} = pwd1.pre_rho(X.Set(ind_X),rho);
        end
    ...
    elseif isa(X,'Polyhedron')
        ...
        for i=1:pwd1.num_region
            pre_from_dyn_i = pwd1.reg_list{i}.intersect(pwd1.dyn_list{i}.pre_rho_rand(X, rho));
            % S = add1(S, new_poly);
