function q = projection2(P, dims, method, solver)
...
if false %P.hasVRep % disabled by zexiang since numerically unstable
	% Easy way - V-rep
	...
	if size(P.He,1) > 0
		...
		switch lower(method)
			...
			case 'vrep'
				...
				Pn.minVRep();
				q = projection2(Pn, dims);
				q = minHRep2(q); % return H-rep, because the entry was in H
				
			case 'fourier'
				Pn = minHRep2(Pn); % Redundancy elimination
